...
def ecg_inference(input_data):
    ...
    if lead == 12:
        ...
        chkpoint = torch.load("D:/workspace_pycharm/ECG-web/ecgWeb/apps/patient/model/sl_model_12.pt", map_location='cpu')
        ...
        chkpoint = torch.load("D:/workspace_pycharm/ECG-web/ecgWeb/apps/patient/model/sl_model_1.pt", map_location='cpu')
...
if __name__ == '__main__':
    ...
    #     print(key)
    model.load_state_dict(pretrained_dict)
    x = torch.randn(1, *input_shape).cuda()  # 假设批大小为 32
    output = model(x)
    print("Output shape:", output.shape)  # torch.Size([1, 4])
    _, predicted_class = torch.max(output, 1)  # 返回每个样本的最大值索引
    predicted_class = predicted_class.cpu().numpy()  # 如果需要的话，转换到CPU并转为numpy数组
